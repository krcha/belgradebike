...
class WC_PB_Display {
	...
	function woo_bundles_frontend_scripts() {
		...
		wp_register_script( 'wc-add-to-cart-bundle', $woocommerce_bundles->woo_bundles_plugin_url() . '/assets/js/add-to-cart-bundle.js', array( 'jquery', 'wc-add-to-cart-variation' ), $woocommerce_bundles->version, true );
		...
		$params = array(
			...
			'i18n_partially_out_of_stock' 	=> __( 'Out of stock', 'woocommerce-product-bundles' ),
	...
	function woo_bundles_empty_price( $price, $product ) {
		...
		if ( ( $product->product_type == 'bundle' ) && ( get_post_meta( $product->id, '_per_product_pricing_active', true ) == 'no' ) )
			return __( 'Price not set', 'woocommerce-product-bundles' );
