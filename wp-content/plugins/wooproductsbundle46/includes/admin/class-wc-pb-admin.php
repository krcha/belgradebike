...
class WC_PB_Admin {
	...
	function woo_bundles_admin_scripts() {
		...
		wp_register_script( 'woo_bundles_writepanel', $woocommerce_bundles->woo_bundles_plugin_url() . '/assets/js/bundled-product-write-panels.js', array( 'jquery', 'jquery-ui-datepicker', $writepanel_dependency ), $woocommerce_bundles->version );
			...
			<div id="bundled_product_data" class="panel woocommerce_options_panel">
				...
				<div class="options_group wc-metaboxes-wrapper wc-bundle-metaboxes-wrapper">
					...
					<div id="wc-bundle-metaboxes-wrapper-inner">
						...
						<p class="toolbar">
							<?php echo '<span class="wc-bundled-items-title">' . __( 'Bundle Configuration', 'woocommerce-product-bundles' ) . '</span>'; echo '<img class="help_tip" data-tip="' . __( 'Use these settings to configure basic bundled product parameters such as quantities / discounts, apply variation filters and default option overrides and fine-tune the appearance of your Bundle.', 'woocommerce-product-bundles' ) . '" src="' . $woocommerce->plugin_url() . '/assets/images/help.png" />'; ?>
									...
									<div class="wc-bundled-item wc-metabox closed" rel="<?php echo $sorting; ?>">
										...
										<div class="item-data wc-metabox-content">
											...
											<div class="item_visibility">
												...
												<p class="tip"><a class="tips" data-tip="<?php _e( 'Hides this bundled product from the front-end. This option will only work if you have defined default selections above.', 'woocommerce-product-bundles' ); ?>" href="#" >[?]</a></p>
												<p class="form-field">
			...
			foreach ( $bundled_product_ids as $id ) {
				...
				if ( ( $id && $id > 0 ) && ( $product_type == 'simple' || $product_type == 'variable' || $is_sub ) && ( $post_id != $id ) ) {
					...
					if ( in_array( $id, $bundled_ids ) && $product_type != 'variable' ) {
						...
						$this->save_errors[] = wc_bundles_add_admin_error( sprintf( __( '\'%s\' (#%s) cannot be bundled in multiple instances. If you need to bundle it more than once, you can adjust its quantity from the \'Bundle Configuration\' options.', 'woocommerce-product-bundles' ), get_the_title( $id ), $id ) );
						continue;
					}
