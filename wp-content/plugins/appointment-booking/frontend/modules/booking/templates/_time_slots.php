...
<?php foreach ( $slots as $slot ) :
    ...
    printf(
        '<button value="%s" data-group="%s" class="bookly-hour%s%s"%s><span class="ladda-label%s"><i class="bookly-hour-icon"><span></span></i>%s%s</span></button>',
        esc_attr( json_encode( $data ) ),
        $group,
        $slot->fullyBooked() ? ' booked' : '',
        $slot->waitingListEverStarted() ? ' bookly-slot-in-waiting-list' : '',
