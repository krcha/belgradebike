...
class Controller extends Lib\Base\Controller
{
    ...
    public function renderShortCode( $attributes )
    {
        ...
        $options = array(
            ...
            'google_map'   => array(
                'enabled'   => (int) ( Lib\Config::googleMapsAddressActive() && Lib\Config::googleMapsAddressEnabled() ),
    ...
    public function executeRenderCart()
    {
        ...
        if ( $userData->load() ) {
            ...
            $response = array(
                'success' => true,
                'html'    => Lib\Proxy\Cart::getStepHtml( $userData, $progress_tracker, $info_text ),
            );
        } else {
            $response = array( 'success' => false, 'error' => Errors::SESSION_ERROR );
        }
        ...
        if ( $userData->load() ) {
            ...
            if ( $payment_disabled == false ) {
                ...
                if ( Lib\Config::payLocallyEnabled() ) {
                    $payments['local'] = $this->render( '_payment_local', array( 'form_id' => $this->getParameter( 'form_id' ) ), false );
                }
                if ( get_option( 'bookly_paypal_enabled' ) ) {
                    $payments['paypal'] = $this->render( '_payment_paypal', array(
                        'form_id' => $this->getParameter( 'form_id' ),
                ...
                $payments = Lib\Proxy\Shared::preparePaymentGatewaySelector( $payments, $this->getParameter( 'form_id' ), $userData->extractPaymentStatus(), $cart_info, Lib\Config::showPaymentSpecificPrices() );
    ...
    public function executeSaveAppointment()
    {
        ...
        if ( $userData->load() ) {
            ...
            if ( $failed_cart_key === null ) {
                ...
                if ( $is_payment_disabled || $is_pay_locally_enabled || $cart_info->getDeposit() <= 0 ) {
                    // Handle coupon.
                    ...
                    if ( ! $is_payment_disabled ) {
                        if ( $coupon && $cart_info->getTotal() <= 0 ) {
                            ...
                            $payment
                                ...
                                ->setPaid( 0 )
                                ->save();
                        } elseif ( $is_pay_locally_enabled && $cart_info->getTotal() > 0 ) {
        ...
        if (
            ...
            if ( $userData->load() ) {
                ...
                if ( $failed_cart_key === null ) {
                    ...
                    $payment   = new Lib\Entities\Payment();
                    $cart_info = Lib\Proxy\Shared::applyPaymentSpecificPrice( $userData->cart->getInfo(), $this->getParameter( 'payment_type' ) );
                    switch ( $this->getParameter( 'payment_type' ) ) {
    ...
    public function executeFacebookLogin()
    {
        ...
        $form_id     = $this->getParameter( 'form_id' );
        $facebook_id = $this->getParameter( 'id' );
        ...
        if ( $userData->load() ) {
            ...
            if ( $customer->loadBy( array( 'facebook_id' => $facebook_id ) ) ) {
                ...
                $user_info  = array(
                    ...
                    'full_name'  => $this->getParameter( 'name' ),
        ...
        if ( get_option( 'bookly_app_show_progress_tracker' ) ) {
            ...
            if ( ! $payment_disabled && $step > Steps::SERVICE ) {
                if ( $step < Steps::CART ) {  // step Cart.
                    ...
                    $cart_info = $userData->cart->getInfo( true );
                    if ( $cart_info->getTotal() == 0 ) {
                        $payment_disabled = true;
                    }
                }
